...
void check_memory_usage() {
    const double MEM_THRESHOLD = 0.6;
    const size_t total_nodes = llmsset_get_size(nodes);
    size_t current_nodes = llmsset_count_marked(nodes);
    double usage_ratio = (double)current_nodes / total_nodes;
    
    if (usage_ratio > MEM_THRESHOLD) {
        sylvan_gc();
        current_nodes = llmsset_count_marked(nodes);
        if (current_nodes > total_nodes * MEM_THRESHOLD) {
            fprintf(stderr, "Warning: High memory usage after GC: %.2f%%\n", 
                    (double)current_nodes / total_nodes * 100);
        }
    }
}
    ...
    while ((c = fgetc(in)) != EOF) {
        ...
        if (strcmp(cmd, "OPENQASM") == 0) {}
            ...
            *bits_to_measure = my_malloc(n * sizeof(int));
            ...
            else if (strcasecmp(cmd, "ccz") == 0) {
                uint32_t qc1 = get_q_num(in);
                uint32_t qc2 = get_q_num(in);
                uint32_t qt = get_q_num(in);
                gate_ccz(circ, qt, qc1, qc2);
            }
