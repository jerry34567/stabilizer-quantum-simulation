...
#include "mtbdd.h"
#include "sylvan_int.h" // for llmsset_count_marked() and llmsset_get_size()
...
int main(int argc, char *argv[])
{
    ...
    char *endptr;
    while((opt = getopt_long(argc, argv, "hif:m::n:p", long_options, 0)) != -1) {
        switch(opt) {
            case 'h':
                printf("%s\n", HELP_MSG);
                exit(0);
    ...
    // size_t total_nodes = llmsset_count_marked(nodes);  // 獲取已使用的節點數
    // size_t table_size = llmsset_get_size(nodes);      // 獲取表大小

    // printf("\nNode statistics:\n");
    // printf("Total nodes in use: %zu\n", total_nodes);
    // printf("Table size: %zu\n", table_size);
    // printf("Table usage: %.2f%%\n", (double)total_nodes/table_size*100);
